...
describe('verificarToken (interactions-service)', () => {
  beforeEach(() => jest.clearAllMocks());

  it('retorna 403 cuando no se proporciona token', () => {
    const { res, status, json } = buildRes();
    verificarToken(buildReq(undefined), res, next);

    expect(status).toHaveBeenCalledWith(403);
    expect(json).toHaveBeenCalledWith({ message: 'Acceso denegado' });
  ...
  it('retorna 401 cuando el token es inválido o expirado', () => {
    ...
    expect(json).toHaveBeenCalledWith({ message: 'Token no válido' });
    expect(next).not.toHaveBeenCalled();
  });
  ...
  it('elimina el prefijo Bearer antes de verificar', () => {
